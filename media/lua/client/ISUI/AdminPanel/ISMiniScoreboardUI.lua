...
function ISMiniScoreboardUI:initialise()
    ...
    self.playerList = ISScrollingListBox:new(20, y, self.width, self.height - (5 + btnHgt + 5) - y);
...
function ISMiniScoreboardUI:populateList()
    ...
    for i=1,self.scoreboard.usernames:size() do
        ...
        if username then
            ...
            local name = displayName .. " (" .. forename .. ")"
            local item0 = self.playerList:addItem(name, item);
            if name then
                item0.tooltip = "Character's Firstname: " .. forename .. "\n" .. "Username: " .. username
            end
        end
    end
end
...
function ISMiniScoreboardUI:drawPlayers(y, item, alt)
    ...
    if self.selected == item.index then
        self:drawRect(0, (y), self:getWidth() - 10, self.itemheight - 1, 0.3, 0.7, 0.35, 0.15);
    end
...
function ISMiniScoreboardUI:prerender()
    local z = 10;
    self:drawRect(0, 0, self.width + 20, self.height, self.backgroundColor.a, self.backgroundColor.r, self.backgroundColor.g, self.backgroundColor.b);
    self:drawRectBorder(0, 0, self.width + 20, self.height, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b);
